...
Definition find_globvar (ge: t) (id: ident) : option (globvar V) :=
  match find_symbol ge id with
  | Some b => find_var_info ge b
  | None => None
  end.
...
Definition size_globvar (gv: globvar V) : Z :=
  init_data_list_size (gv.(gvar_init)).
...
(* JBA: added/adapted from more recent versions of compcert *)
...
Record unchanged_on (m_before m_after: mem) : Prop := mk_unchanged_on {
  ...
  unchanged_on_perm:
    ...
    (Mem.perm m_before b ofs k p <-> Mem.perm m_after b ofs k p);
...
Proof.
  unfold Mem.valid_block; intros. apply unchanged_on_nextblock in H. xomega.
Qed.
...
Proof.
  intros. destruct H. apply unchanged_on_perm0; auto. eapply Mem.perm_valid_block; eauto.
Qed.
...
- intros. transitivity (Mem.perm m2 b ofs k p); apply unchanged_on_perm; auto.
...
Proof.
  intros.
  destruct (zle n 0).
+ erewrite ! Mem.loadbytes_empty by assumption. auto.
+ Transparent Mem.loadbytes. unfold Mem.loadbytes. destruct H.
  destruct (Mem.range_perm_dec m b ofs (ofs + n) Cur Readable).
  rewrite pred_dec_true. f_equal.
  apply Mem.getN_exten. intros. rewrite nat_of_Z_eq in H by omega.
...
Proof.
  intros.
  destruct (zle n 0).
+ erewrite Mem.loadbytes_empty in * by assumption. auto.
+ rewrite <- H1. apply loadbytes_unchanged_on_1; auto.
  exploit Mem.loadbytes_range_perm; eauto. instantiate (1 := ofs). omega.
...
- rewrite (Mem.nextblock_store _ _ _ _ _ _ H). apply Ple_refl.
...
- split; intros. eapply Mem.perm_storebytes_1; eauto. eapply Mem.perm_storebytes_2; eauto.
- erewrite Mem.storebytes_mem_contents; eauto. rewrite PMap.gsspec.
  destruct (peq b0 b); auto. subst b0. apply Mem.setN_outside.
  destruct (zlt ofs0 ofs); auto.
...
- split; intros.
  eapply Mem.perm_alloc_1; eauto.
  eapply Mem.perm_alloc_4; eauto.
  eapply Mem.valid_not_valid_diff; eauto with mem.
-  Transparent Mem.alloc. injection H; intros A B. rewrite <- B; simpl.
...
Proof.
  intros; constructor; intros.
- rewrite (Mem.nextblock_drop _ _ _ _ _ _ H). apply Ple_refl.
- split; intros. eapply Mem.perm_drop_3; eauto.
...
- unfold Mem.drop_perm in H.
  destruct (Mem.range_perm_dec m b lo hi Cur Freeable); inv H; simpl. auto.
Qed.
...
Remark init_data_size_addrof:
  forall id ofs, init_data_size (Init_addrof id ofs) = size_chunk Mint32.
Proof.
  intros. simpl. auto.
Qed.
...
- rewrite init_data_size_addrof.
  destruct (find_symbol ge i) as [b'|] eqn:?; try discriminate.
  rewrite (Mem.loadbytes_store_same _ _ _ _ _ _ H).
  unfold encode_val, bytes_of_init_data; rewrite Heqo; reflexivity.
Qed.
...
Remark store_zeros_unchanged:
  forall (P: block -> Z -> Prop) m b p n m' (Hb: Mem.valid_block m b),
  store_zeros m b p n = Some m' ->
...
+ eapply store_unchanged_on; eauto. simpl. intros. apply H0. omega.
+ apply IHo; auto. eapply Mem.store_valid_block_1; eauto. intros; apply H0; omega.
- discriminate.
Qed.
...
- generalize (init_data_size_pos i1) (init_data_list_size_pos il); intros P1 PL.
  ...
  apply Mem.loadbytes_concat.
  eapply loadbytes_unchanged_on with (P := fun b1 ofs1 => ofs1 < p + init_data_size i1).
  ...
  red; intros.
  eapply loadbytes_unchanged_on with (P := fun b1 ofs1 => p + init_data_size i1 <= ofs1).
...
Definition variables_initialized (g: t) (m: mem) :=
  ...
  /\ (gv.(gvar_volatile) = false -> Mem.loadbytes m b 0 (init_data_list_size gv.(gvar_init)) = Some (bytes_of_init_data_list gv.(gvar_init))).
...
(* function *)
  ...
  exploit H1; eauto. intros [A [B [C D]]].
  ...
  intros. unfold alloc_global in H0.
  erewrite (@Mem.loadbytes_drop (fst (Mem.alloc m 0 1))).
  2: apply H0.
  erewrite (@Mem.loadbytes_alloc_unchanged m 0 1 _ (Mem.alloc m 0 1)#2); eauto.  
  left; apply Plt_ne. apply E.
(* variable *)
  ...
  split. intros. apply load_store_init_data_invariant with m3.
  ...
  intros GVvol.
  erewrite (@Mem.loadbytes_drop m3); eauto.
  apply store_init_data_list_loadbytes with m2; auto.
  red. eapply store_zeros_loadbytes. apply Heqo.
  right; right; right. unfold perm_globvar; rewrite GVvol.
  destruct (gvar_readonly gv); auto with mem.
  (* older var *)
  exploit H1; eauto. intros [A [B [C D]]].
  assert (E: Mem.valid_block m b).
  ...
  split. intros. apply load_store_init_data_invariant with m; auto.
  intros. eapply load_alloc_global; eauto.
  intros. eapply loadbytes_unchanged_on with (P := fun b1 ofs1 => 0 <= ofs1 < 0 + init_data_list_size (gvar_init gv)) (m:=m); eauto.
  eapply alloc_global_unchanged; eauto.
...
Theorem init_mem_characterization':
  ...
  /\ (gv.(gvar_volatile) = false -> Mem.loadbytes m b 0 (init_data_list_size gv.(gvar_init)) = Some (bytes_of_init_data_list (globalenv p) gv.(gvar_init))).
Proof.
...
Proof.
intros.
generalize (@init_mem_characterization' p b gv m H H0); intros (A & B & C & D).
split; auto.
Qed.
