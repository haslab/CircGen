...
let compile_c_ast sourcename csyntax ofile debug =
  ...
  set_dest PrintRTLC.destination option_drtlc ".rtlc";
  set_dest PrintValCirc.destination option_dvalcirc ".valc";
  (*set_dest PrintHLCirc.destination option_dhlc ".hlc";*)
  if !option_dhlc || not (!option_dllc || !option_bristolsmc)
  then PrintHLCirc.destination := Some (output_filename sourcename ".c" ".hlc")
  else PrintHLCirc.destination := None;
  set_dest PrintLLCirc.destination option_dllc ".llc";
  set_dest PrintBristolCirc.destination option_bristolsmc ".circ";
  set_dest ProcessHLCirc.unittest_file option_unittest ".tests";
  (* Convert to HLL *)
  let hlc =
    ...
    | Errors.Error msg ->
        ...
        exit 2 in
  PrintHLCirc.print_if hlc;
  ProcessHLCirc.process_program hlc

(* From C source to RTL *)
...
let compile_cminor_file ifile ofile =
  ...
  try
    ...
    | Errors.Error msg ->
        ...
        exit 2
    | Errors.OK p ->
        (*RTL2Circ.print_circ p ofile*)()
        (*let oc = open_out ofile in
          RTL2Circ.print_program oc p;
          close_out oc*)
  ...
  if !option_E then begin
    ...
    if !option_interp then begin
      ...
      compile_c_file sourcename preproname
                     (output_filename ~final:true sourcename ".c" ".final");
      safe_remove preproname;
      ""
    end 
  end
...
let process_cminor_file sourcename =
  begin
    compile_cminor_file sourcename
                        (output_filename ~final:true sourcename ".cm" ".final");
    ""
  end 
...
Code generation options: (use -fno-<opt> to turn off -f<opt>)
  ...
  -fce          Perform consant expansion (no machine-checked proof) [off]
  -floop_unroll <n>          Set number of loop unfolds
  -floop_unroll_infer        Infers the number of loop unfolds
Tracing options:
  ...
  -drtlc         Save RTLC at various optimization points in <file>.rtlc.<n>
  -dvalcirc         Save ValCirc in <file>.valc
  -dhlc          Save generated HLC code in <file>.hlc
  -dllc          Save generated LLC code in <file>.llc
Circuit Generation options:
  -bristol-smc   Save generated LLC in Bristol SMC format in <file>.circ
  -p1-inputs <n> Assign <n> input wires to Party1 (in Bristol SMC format)
  -show-stats    Show statics on the generated circuit
General options:
...
let optimization_options = [
  option_fconstprop; option_fcse; option_fredundancy; option_fce
]
...
(* Code generation options -- more below *)
  ...
  Exact "-floop_unroll", Integer(fun n -> option_floop_unroll := n);
  Exact "-floop_unroll_infer", Set option_floop_unroll_infer;
(* Tracing options *)
  ...
  Exact "-drtlc", Set option_drtlc;
  Exact "-dvalcirc", Set option_dvalcirc;
  Exact "-dhlc", Set option_dhlc;
  Exact "-dllc", Set option_dllc;
  Exact "-bristol-smc", Set option_bristolsmc;
  Exact "-p1inputs", Integer(fun n -> option_p1inputs := n);
  Exact "-show-stats", Set option_showstats;
  Exact "-selftest", Integer(fun n -> option_selftest := n);
  Exact "-unittest", Set option_unittest;
  Exact "-dbglevel", Integer(fun n -> option_dbglevel := n);
  Exact "-noxpnd", Set option_noxpnd;
  Exact "-nomemo", Set option_nomemo;
  Exact "-nosimpl", Set option_nosimpl;
(* General options *)
...
(* File arguments *)
  Suffix ".c", Self (fun s ->
    c_file_name := s; incr num_source_files; incr num_input_files) (*
...
let _ =
  try
    ...
    time "Total compilation time" (*perform_actions ()*)
	 process_c_file !c_file_name
  with Sys_error msg ->
    eprintf "I/O error: %s.\n" msg; exit 2
