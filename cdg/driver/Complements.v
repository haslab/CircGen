...
Theorem transf_c_program_is_refinement:
  ...
  (forall beh, program_behaves (RTL.semantics tp) beh -> program_behaves (Csem.semantics p) beh).
Proof.
...
Theorem transf_cstrategy_program_preservation:
  ...
  (forall beh, program_behaves (Cstrategy.semantics p) beh ->
     exists beh', program_behaves (RTL.semantics tp) beh' /\ behavior_improves beh beh')
...
/\(forall beh, not_wrong beh ->
     program_behaves (Cstrategy.semantics p) beh -> program_behaves (RTL.semantics tp) beh)
/\(forall beh,
...
Theorem bigstep_cstrategy_preservation:
  ...
  (forall t r,
     ...
     program_behaves (RTL.semantics tp) (Terminates t r))
/\(forall T,
     Cstrategy.bigstep_program_diverges p T ->
       program_behaves (RTL.semantics tp) (Reacts T)
    \/ exists t, program_behaves (RTL.semantics tp) (Diverges t) /\ traceinf_prefix t T).
Proof.
  intuition.
...
Theorem transf_c_program_preserves_spec:
  ...
  (forall beh, program_behaves (RTL.semantics tp) beh -> spec beh).
Proof.
  intros.
...
Theorem transf_c_program_preserves_liveness_spec:
  ...
  (forall beh, program_behaves (RTL.semantics tp) beh -> liveness_spec_satisfied beh).
Proof.
  intros. eapply transf_c_program_preserves_spec; eauto.
