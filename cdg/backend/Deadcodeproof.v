...
- (* store *)
  TransfInstr; UseTransfer.
  destruct (nmem_contains nm (aaddressing (_ (vanalyze rm f) # pc) addr args)
...
- (* builtin *)
  ...
  functional induction (transfer_builtin (Fiber.vat_at pc (vanalyze rm f)#pc) ef args res ne nm);
  simpl in *; intros.
+ (* volatile load *)
  inv H0. inv H6. rename b1 into v1.
  destruct (transfer_builtin_arg All
              (kill_builtin_res res ne,
              nmem_add nm (aaddr_arg (_ (vanalyze rm f) # pc) a1)
...
+ (* memcpy *)
  rewrite e1 in TI.
  inv H0. inv H6. inv H7. rename b1 into v1. rename b0 into v2.
  set (adst := aaddr_arg (_ (vanalyze rm f) # pc) dst) in *.
  set (asrc := aaddr_arg (_ (vanalyze rm f) # pc) src) in *.
  destruct (transfer_builtin_arg All
              (kill_builtin_res res ne,
...
+ (* memcpy eliminated *)
  ...
  inv H0. inv H6. inv H7. rename b1 into v1. rename b0 into v2.
  set (adst := aaddr_arg (_ (vanalyze rm f) # pc) dst) in *.
  set (asrc := aaddr_arg (Fiber.vat_at pc (vanalyze rm f) # pc) src) in *.
  inv H1.
  econstructor; split.
  eapply exec_Inop; eauto.
  eapply match_succ_states; eauto. simpl; auto.
