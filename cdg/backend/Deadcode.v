...
Definition transfer (f: function) (approx: PMap.t Fiber.t)
  ...
  | Some (Iload chunk addr args dst s) =>
      ...
      if is_dead ndst then after
            ...
            nmem_add nm (aaddressing (Fiber.vat_at pc (approx!!pc)) addr args) (size_chunk chunk))
  | Some (Istore chunk addr args src s) =>
      let p := aaddressing (Fiber.vat_at pc (approx!!pc)) addr args in
  ...
  | Some(Ibuiltin ef args res s) =>
      transfer_builtin (Fiber.vat_at pc (approx!!pc)) ef args res ne nm
...
Definition analyze (approx: PMap.t Fiber.t) (f: function): option (PMap.t NA.t) :=
  DS.fixpoint f.(fn_code) successors_instr
              (transfer f approx).

(** * Part 2: the code transformation *)

Definition transf_instr (approx: PMap.t Fiber.t) (an: PMap.t NA.t)
  ...
  | Ibuiltin (EF_memcpy sz al) (dst :: src :: nil) res s =>
      if nmem_contains (snd an!!pc) (aaddr_arg (Fiber.vat_at pc (approx!!pc)) dst) sz
      then instr
      else Inop s
  | _ =>
      instr
