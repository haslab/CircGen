...
Definition reg_valnum (n: numbering) (vn: valnum) : option reg :=
  match rev' (PMap.get vn n.(num_val)) with
  | nil => None
  | r :: rs => Some r
  end.
...
Definition transfer (f: function) (approx: PMap.t Fiber.t) (pc: node) (before: numbering) :=
  ...
  | Some i =>
      ...
      | Istore chunk addr args src s =>
          let app := Fiber.vat_at pc (approx!!pc) in
...
Definition analyze (f: RTL.function) (approx: PMap.t Fiber.t): option (PMap.t numbering) :=
...
Definition reduce_reg (n: numbering) (r: reg) : numbering * reg :=
  let '(n', v) := valnum_reg n r in
  let r' := match reg_valnum n' v with Some x => x | None => r end in
  (n', r').

Definition reduce_builtin_args ef (n: numbering) (args: list (builtin_arg reg)) :
  numbering * list (builtin_arg reg) :=
  match ef with
  | EF_vstore _ =>
  match args with
  | x :: BA r :: nil => let '(n, r) := reduce_reg n r in (n, x :: BA r :: nil)
  | _ => (n, args)
  end
  | _ => (n, args)
  end.
...
Definition transf_instr (n: numbering) (instr: instruction) :=
  ...
  | Icond cond args s1 s2 =>
      ...
      Icond cond' args' s1 s2
  | Ibuiltin ef args res s =>
    let '(n, args) := reduce_builtin_args ef n args in
    Ibuiltin ef args res s
  | _ =>
      instr
  end.
