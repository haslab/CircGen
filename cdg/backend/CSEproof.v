...
Proof.
  ...
  apply in_rev.
  destruct (rev _). discriminate. left. congruence.
Qed.
...
Import Utf8.
Lemma reduce_reg_sound {valu sp rs m n r n' r'} :
  numbering_holds valu ge sp rs m n →
  reduce_reg n r = (n', r') →
  ∃ valu' v,
    numbering_holds valu' ge sp rs m n' ∧
    rs # r = valu' v ∧
    rs # r' = valu' v ∧
    valu_agree valu valu' (num_next n) ∧
    Plt v (num_next n') ∧
    Ple (num_next n) (num_next n').
Proof.
  intros NH.
  unfold reduce_reg.
  generalize (λ n' v, valnum_reg_holds _ _ _ _ _ _ r n' v NH).
  destruct (valnum_reg n r) as (n0, v).
  intros H. specialize (H _ _ eq_refl).
  destruct H as (valu' & NH' & Hr & VA & LT & LE).
  intros H; inv H.
  generalize (λ r' H, reg_valnum_sound n' v r' _ _ _ _ _ H NH').
  destruct (reg_valnum _ _) as [ r' | ].
  intros H; specialize (H _ eq_refl); eauto 8.
  intros _; eauto 8.
Qed.

Lemma reduce_builtin_args_eval {valu sp rs m ef n args vargs n' args'} :
  numbering_holds valu ge sp rs m n →
  eval_builtin_args ge (λ x, rs # x) sp m args vargs →
  reduce_builtin_args ef n args = (n', args') →
  eval_builtin_args ge (λ x, rs # x) sp m args' vargs ∧
  ∃ valu', numbering_holds valu' ge sp rs m n'.
Proof.
(*
  revert vargs n' args'.
  elim args; clear.
  intros vargs n' args' NH EV H; inv H. eauto.
  intros ba args IH vargs n' args' NH EV.
  simpl. destruct (reduce_builtin_args _ _) as (ni, argsi).
  inv EV.
  match goal with
  | H : list_forall2 _ _ _ |- _ => specialize (IH _ _ _ NH H eq_refl)
  end.
  destruct IH as (IH & valu' & NH').
  simpl.
  destruct ba;
    try (intros H; inv H; constructor; eauto; constructor; eauto; fail).
  destruct (reduce_reg _ _) as (n0 & r) eqn: EQ; intros H; inv H.
  generalize (reduce_reg_sound NH' EQ).
  intros (valu0 & v & NH0 & Hv & Hr & VA & LT & LE).
  split. constructor.
  match goal with H : eval_builtin_arg _ _ _ _ _ _ |- _ => inv H end.
  rewrite Hv, <- Hr. constructor.
  exact IH. eauto.
 *)
  destruct ef; try (intros ? ? X; inv X; eauto; fail).
  destruct args as [ | ? [ | () [ | ? ? ] ] ];
    try (intros ? ? X; inv X; eauto; fail).
  intros NH EV H; simpl in H.
  destruct (reduce_reg _ _) as (n0, r') eqn: EQ.
  inv H.
  generalize (reduce_reg_sound NH EQ).
  intros (valu' & v & NH' & Hv & Hr & VA & LT & LE).
  inv EV.
  split. constructor; auto.
  match goal with H : list_forall2 _ _ _ |- _ => inv H end.
  constructor.
  match goal with H : eval_builtin_arg _ _ _ _ _ _ |- _ => inv H end.
  rewrite Hv, <- Hr. constructor.
  assumption. eauto.
...
- (* Istore *)
  destruct SAT as [valu0 NH0].
  destruct (reduce_reg _ _) as (n0, src') eqn: Hsrc.
  generalize (reduce_reg_sound NH0 Hsrc).
  intros (valu1 & vs & NH1 & EQ1 & EQ2 & AG1 & P1 & Q1).
  destruct (valnum_regs _ args) as [n1 vl] eqn:?.
  exploit valnum_regs_holds; eauto. intros (valu2 & NH2 & EQ & AG & P & Q).
  destruct (reduce addressing combine_addr n1 addr args vl) as [addr' args'] eqn:?.
  assert (ADDR: eval_addressing ge sp addr' rs##args' = Some a).
  { eapply reduce_sound with (sem := fun addr vl => eval_addressing ge sp addr vl); eauto.
  ...
  assert (ADDR': eval_addressing tge sp addr' rs'##args' = Some a').
  { rewrite <- A. apply eval_addressing_preserved. exact symbols_preserved. }
  generalize (RLD src'). rewrite EQ2, <- EQ1. intros LD.
  generalize (Mem.storev_extends _ _ _ _ _ _ _ _ MEXT H1 B LD).
  intros [m'' [X Y]].
  econstructor; split.
...
- (* Ibuiltin *)
  ...
  destruct (reduce_builtin_args _ _ _) as (n' & args') eqn: Hargs.
  match goal with
  | H : eval_builtin_args _ _ _ _ _ _ |- _ =>
    generalize (reduce_builtin_args_eval NH H Hargs); clear H
  end.
  intros (EV & valu' & NH').
...
* unfold transfer; rewrite H.
  ...
  + apply CASE1.
  + apply CASE1.
  + destruct args as [ | dst [ | src [ | ? ? ] ] ]; auto.
    assert (args' = dst :: src :: nil).
    { destruct dst; inv Hargs; reflexivity. }
    subst args'.
    exists valu.
    apply set_res_unknown_holds.
    inv EV. inv H5. inv H7. inv H1.
    inv SOUND. unfold vanalyze, rm; rewrite AN.
    assert (pmatch bc bsrc osrc (aaddr_arg (VA.State ae am) src))
    by (eapply aaddr_arg_sound_1; eauto).
    assert (pmatch bc bdst odst (aaddr_arg (VA.State ae am) dst))
    by (eapply aaddr_arg_sound_1; eauto).
