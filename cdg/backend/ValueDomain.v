...
(** Condition move operation (phi-node) *)

Definition cmove (x y z: aval) :=
  match x with
  | I i => if Int.eq i Int.zero then z else y
  | _ => vlub y z
  end.

Lemma cmove_sound:
  forall u v w x y z, 
  vmatch u x -> vmatch v y -> vmatch w z -> 
  vmatch (if Val.eq u (Val.of_bool false) then w else v) (cmove x y z).
Proof.
  intros. destruct (Val.eq u (Val.of_bool false)).
   simpl in e; unfold cmove. inv H; try solve[apply  vmatch_lub_r; auto].
   assert (Int.eq i Int.zero = true) by (inv H2; apply Int.eq_true).
   rewrite H; simpl; auto.
  unfold cmove. inv H; try solve[apply  vmatch_lub_l; auto].
  assert (Int.eq i Int.zero = false). 
   apply Int.eq_false; intro HH; apply n.
   simpl; unfold Vfalse; f_equal; auto.
  rewrite H; auto.
Qed.
