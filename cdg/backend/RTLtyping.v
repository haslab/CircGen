...
(** from Conventions.v file
 remark: under the parameter passing conventions we have based our
 tool (ia32), all arguments are passed on stack, turning taicall
 not possible.
*)
Definition tailcall_possible (s: signature) : Prop := False.

Definition tailcall_is_possible (sg: signature) : bool := false.
...
- (* tailcall *)
  ...
  revert H3. unfold tailcall_possible, tailcall_is_possible. 
   intro HH; elim HH.
  inv H3.
(*
  generalize (loc_arguments sig).
