...
let print_addressing reg pp = function
  | Aindexed n, [r1] ->
      fprintf pp "(@I %a + %ld)" reg r1 (camlint_of_coqint n)
  | Aindexed2 n, [r1; r2] ->
      fprintf pp "(@I2 %a + %a + %ld)" reg r1 reg r2 (camlint_of_coqint n)
  | Ascaled(sc,n), [r1] ->
      fprintf pp "(@S %a * %ld + %ld)" reg r1 (camlint_of_coqint sc) (camlint_of_coqint n)
  | Aindexed2scaled(sc, n), [r1; r2] ->
      fprintf pp "(@I2S %a + %a * %ld + %ld)" reg r1 reg r2 (camlint_of_coqint sc) (camlint_of_coqint n)
  | Aglobal(id, ofs), [] -> fprintf pp "(@G[<%ld] %s + %ld)" (get_globvar_size id) (extern_atom id) (camlint_of_coqint ofs)
  | Abased(id, ofs), [r1] -> fprintf pp "(@B %s + %ld + %a)" (extern_atom id) (camlint_of_coqint ofs) reg r1
  | Abasedscaled(sc,id, ofs), [r1] -> fprintf pp "(@BS %s + %ld + %a * %ld)" (extern_atom id) (camlint_of_coqint ofs) reg r1 (camlint_of_coqint sc)
